...
const createLocation = (req, res) => {
  db.locations.create(req.body).then((createdLocation) => {
    if (!createdLocation) {
      ...
      res
        .status(201)
        .json({ data: createdLocation, message: "location created" });
    }
  });
};

const getLocations = (req, res) => {
  db.locations.find({}).then((foundLocations) => {
    if (!foundLocations) {
  ...
  db.locations
    ...
    .then((updatedLocation) => {
      if (!updatedLocation) {
        ...
        res
          .status(200)
          .json({ Data: updatedLocation, Message: "location updated" });
      }
    });
};

const deleteLocation = (req, res) => {
  db.locations.findByIdAndDelete(req.params.id).then((deletedLocation) => {
    if (!deletedLocation) {
      ...
      res
        .status(200)
        .json({ Data: deletedLocation, Message: "location deleted" });
    }
  });
};
