...
const logoutUser = async (req, res) => {
  try {
    ...
    const checkIfBlacklisted = await db.cookieBlacklist.findOne({
      token: accessToken,
    });
...
const getCurrentUserInfo = async function (req, res, next) {
  ...
  if (header) {
    ...
    jwt.verify(cookie, accessToken, async (err, decoded) => {
      ...
      const { id } = decoded;
      const user = await db.users.findById(id).then((res) => {
        return res;
      });
      const userData = {
        id: user.id,
  ...
  if (header) {
    ...
    if (checkIfBlacklisted) {
      ...
      res.status(400).json({
        ...
        data: [userData],
        message:
          "Attempting to log in with a logged out token, reauthenticate.",
      });
    }
    jwt.verify(cookie, accessToken, async (err, decoded) => {
      if (err) {
